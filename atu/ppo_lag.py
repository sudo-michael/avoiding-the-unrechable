    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                value_cost = agent.get_cost(next_obs)
            ...
            for item in info:
                if "episode" in item.keys():
                    print(
                        f"global_step={global_step}, episodic_return={item['episode']['r']} episodic_cost={item['episode']['c']}"
                    )
                    writer.add_scalar(
        ...
        # cost values
        with torch.no_grad():
            next_value_cost = agent.get_cost(next_obs).reshape(1, -1)
            if args.gae:
                ...
                for t in reversed(range(args.num_steps)):
                    if t == args.num_steps - 1:
                        nextnonterminal = 1.0 - next_done
                        nextvaluescost = next_value_cost
                    else:
                        nextnonterminal = 1.0 - dones[t + 1]
                        nextvaluescost = costs[t + 1]
                    delta = (
                        costs[t]
                        + args.gamma * nextvaluescost * nextnonterminal
                        - costs[t]
                    )
                    advantages_cost[t] = lastgaelam = (
                        delta
                        + args.gamma * args.gae_lambda * nextnonterminal * lastgaelam
                    )
                returns_cost = advantages_cost + costs
            else:
                returns_cost = torch.zeros_like(costs).to(device)
                for t in reversed(range(args.num_steps)):
                    if t == args.num_steps - 1:
                        nextnonterminal = 1.0 - next_done
                        next_cost = next_value_cost
                    else:
                        nextnonterminal = 1.0 - dones[t + 1]
                        next_cost = costs[t + 1]
                    costs[t] = costs[t] + args.gamma * nextnonterminal * next_cost
                advantages_cost = costs - values_cost
