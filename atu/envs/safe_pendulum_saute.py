...
class SafePendulumEnv(gym.Env, SafeEnv):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        self,
        g=10.0,
        matlab_controller=None,
        done_if_unsafe=False,
        unsafe_lower=np.pi / 4,
        unsafe_upper=np.pi / 2,
    ):
    ...
    def step(self, u):
        ...
        if self.done_if_unsafe and not self.is_safe(newth):
            done = True

        return self._get_obs(), -costs, done, {"safe": self.is_safe(newth)}
    ...
    def render(self, mode="human"):
        ...
        left_arch = min(
            int(self.unsafe_lower * 180 / np.pi + 90),
            int(self.unsafe_upper * 180 / np.pi + 90),
        )
        right_arch = max(
            int(self.unsafe_lower * 180 / np.pi + 90),
            int(self.unsafe_upper * 180 / np.pi + 90),
...
if __name__ in "__main__":
    ...
    # env = SafePendulumEnv(
    #     # unsafe_lower=np.pi / 2,
    #     # unsafe_upper=np.pi * 6 / 5 + np.pi / 2 + np.pi / 8 + np.pi / 16,
    #     # unsafe_upper=0,
    # )
    import gym
    import atu
