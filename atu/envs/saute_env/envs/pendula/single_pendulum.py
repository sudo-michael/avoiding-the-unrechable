...
import torch
from os import path
from atu.envs.saute_env.envs.utils import angle_normalize, Array

from atu.envs.saute_env.envs.wrappers.saute_env import saute_env
from atu.envs.saute_env.envs.wrappers.safe_env import SafeEnv
...
class SafePendulumEnv(gym.Env):
    ...
    def __init__(self, params: Callable = None, mode: str = "train"):
        self.viewer = None
        if params is None:
    ...
    def do_simulation(self, u: np.ndarray):
        ...
        newthdot = (
            thdot
            + (
                -3 * self.params.g / (2 * self.params.l) * np.sin(th + np.pi)
                + 3.0 / (self.params.m * self.params.l ** 2) * u
            )
            * dt
        )
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        info = dict()
        info["cost"] = self._safety_cost_fn(obs, action, next_obs)
    ...
    def render(self, mode="human"):
        ...
        left_arch = min(
            int(self.params.unsafe_min * 180 / np.pi + 90),
            int(self.params.unsafe_max * 180 / np.pi + 90),
        )
        right_arch = max(
            int(self.params.unsafe_min * 180 / np.pi + 90),
            int(self.params.unsafe_max * 180 / np.pi + 90),
        ...
        left_arch = min(
            int(
                (self.params.unsafe_min - self.params.hazard_area_size) * 180 / np.pi
                + 90
            ),
            int(
                (self.params.unsafe_max + self.params.hazard_area_size) * 180 / np.pi
                + 90
            ),
        )
        right_arch = max(
...
if "__main__" in __name__:
    ...
    env = SautedPendulumEnv(mode="deterministic")
    env.reset()
    for _ in range(100):
