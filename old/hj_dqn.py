...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train-frequency", type=int, default=1, help="the frequency of training"
    )
    parser.add_argument(
        "--save_model", type=bool, default=False, help="save model after training"
    )
    parser.add_argument(
        "--save_freq", type=int, default=10_000, help="save every global steps"
    )
    parser.add_argument(
        "--eval", type=bool, default=False, help="path to saved model", required=False
    )
    parser.add_argument(
        "--eval_path",
        type=str,
...
experiment_name = (
    f"{args.gym_id}__{args.exp_name}__{args.seed}__{int(time.time())}_eval{args.eval}"
)
...
class QNetwork(nn.Module):
    ...
    def save_checkpoint(self, exp_name, global_step, ckpt_path=None):
        ...
        if ckpt_path is None:
            ckpt_path = "checkpoints/{}/dqn_checkpoint_{}.pth".format(
                exp_name, global_step
            )
...
for global_step in range(args.total_timesteps):
    ...
    if (
        ...
        with torch.no_grad():
            target_max = torch.max(target_network.forward(s_next_obses, device), dim=1)[
                0
            ]
        ...
        old_val = (
            q_network.forward(s_obs, device)
            .gather(1, torch.LongTensor(s_actions).view(-1, 1).to(device))
            .squeeze()
        )
